...
class BasicAgentAA(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        path= self.next_move(pacman,food,maze)
        print "\nShortest Path: ", path
        print "Number oif moves: ", len(path)-1

        print "\nNumber of actions", self.countActions

        if path[1][1] > pacman[1]:
            move = Directions.NORTH
        if path[1][1] < pacman[1]:
            move = Directions.SOUTH
        if path[1][0] > pacman[0]:
    ...
    def PrintLine(self,gameState):
        string = " ### Pacman position: " + str(gameState.getPacmanPosition())
        string += " | Number of ghosts: "+ str(gameState.getNumAgents() - 1)
        string += " | Living ghosts: "+ str(gameState.getLivingGhosts())
        string += " | Ghosts positions: "+ str(gameState.getGhostPositions())
        return string
