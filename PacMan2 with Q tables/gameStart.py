...
class Game:
    ...
    def run( self ):
        ...
        while not self.gameOver:
            ...
            if agentIndex == 0: #pacman is selected

                agent.writeQtable()
                action = agent.getAction(self.state)
                nextGameState = self.state.generateSuccessor(agentIndex, action)
                #nextGameState = agent.getNext(self.state, action)
                reward = agent.getReward(self.state, nextGameState)
                agent.update(self.state, action, nextGameState, reward)
